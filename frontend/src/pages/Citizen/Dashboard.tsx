...
export default function Dashboard() {
  ...
  const { user } = useAuth();
  const [stats, setStats] = useState({ activeAlerts: 4, openReports: 12, rescueRequests: 2 });
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        ...
        const rescueCount = rescues.filter((req) => req.status.toLowerCase() !== "resolved").length;
