  ...
  return (
    <main className="min-h-screen text-(--color-dark-teal) space-y-6">
      <div className="rounded-3xl sm:rounded-4xl border border-red-200 bg-white p-6 sm:p-10 shadow-[0_20px_70px_rgba(239,68,68,0.12)]">
        ...
        <h1 className="mt-2 text-3xl sm:text-4xl font-black text-(--color-deep-ocean)">
          Request Immediate Emergency Assistance
        </h1>

        <p className="mt-2 max-w-2xl text-sm sm:text-base leading-relaxed text-(--color-medium-teal)">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 max-w-4xl">
          ...
          <div className="flex flex-col justify-between rounded-3xl border-2 border-(--color-ocean) bg-(--color-soft-mint)/40 p-6 sm:p-8 shadow-sm hover:shadow-md transition">
            <div className="space-y-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-(--color-ocean) text-white shadow-sm">
                <LifeBuoy className="h-6 w-6" />
              </div>
              <h2 className="text-xl font-bold text-(--color-deep-ocean)">
                Submit Detailed Rescue Request
              </h2>
              <p className="text-xs sm:text-sm text-(--color-medium-teal) leading-relaxed">
            ...
            <button
              ...
              className="mt-6 flex items-center justify-center gap-2 w-full rounded-2xl bg-(--color-ocean) px-6 py-4 text-sm font-bold text-white shadow-md hover:bg-(--color-deep-ocean) transition active:scale-98"
            >
              Ask for Help
        ...
        <div className="mt-8 rounded-2xl border border-[rgba(53,98,103,0.14)] bg-(--color-pale-aqua)/25 p-5 max-w-4xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <PhoneCall className="h-5 w-5 text-(--color-ocean) shrink-0" />
            <div>
          ...
          <a
            href="tel:1070"
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2 text-xs font-bold text-(--color-ocean) border border-(--color-ocean)/30 hover:bg-(--color-soft-mint) transition"
          >
