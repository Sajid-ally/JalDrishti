...
export default function RescueRelief() {
  ...
  const activeTab =
    searchParams.get("tab") === "status" ? "status" : "request";
    ...
    try {
      ...
      if (data.length > 0) {
        setSelectedReq((prev) => {
          ...
          return (
            data.find((item) => item.id === prev.id) ??
            data[0]
          );
        });
      } else {
  ...
  const handleSubmit = async (
    ...
    try {
      /*
       * Backend requires latitude and longitude.
       * We silently fetch the user's current location.
       * The existing UI/design remains unchanged.
       */
      const coordinates =
        await getCurrentCoordinates();

      /*
       * Payload matches the FastAPI ReliefRequestCreate
       * contract.
       */
      const payload = {
        title: requestType,
         ...
         * Add the logged-in username here later
         * when authentication integration is available.
         */
      };
      ...
      /*
       * Backend response is already persisted.
       * Find the newly-created request from the
       * refreshed backend list.
       */
      setSelectedReq(() => {
        const latest =
          requests.find(
